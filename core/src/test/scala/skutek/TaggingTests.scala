...
class TaggingTests extends Specification {
  ...
  def is = JustOne.is ^ Mixed.is ^ OneEffectTwoTags.is


  object JustOne {
    def is = br ^ "Single effect tagged" ! {
      val eff = for {
        _ <- Tell("foo") @! TagA
        _ <- Tell("bar") @! TagA
      } yield ()

      val h = WriterHandler.strings @! TagA
  ...
  object Mixed {
    def is = br ^ "Composition of tagged and untagged version of the same effect" ! {
      val eff = for {
        _ <- Tell("foo")
        _ <- Tell("bar") @! TagA
      } yield ()
  ...
  object OneEffectTwoTags {
    def is = br ^ "Composition of 2 instances of a single effect, each with unique tag" ! {
      val eff = for {
        s <- Get[String] @! TagA
        i <- Get[Int] @! TagB
